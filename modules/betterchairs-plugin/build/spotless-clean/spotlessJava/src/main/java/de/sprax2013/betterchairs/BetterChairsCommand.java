...
public class BetterChairsCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("toggleChairs")) {
            ...
            if (args[0].equalsIgnoreCase("toggle")) {
                ...
                if (sender.hasPermission(this.permsReload)) {
                    if (Messages.reload()) {
                        ...
                        sender.sendMessage(Messages.getPrefix()
                                + " §cCould not reload §6messages.yml §7- §cCheck server logs for more information");
                    }
                    ...
                    if (Settings.reload()) {
                        ...
                        sender.sendMessage(Messages.getPrefix()
                                + " §cCould not reload §6config.yml §7- §cCheck server logs for more information");
                    }
                } else {
                    sender.sendMessage(Messages.getString(Messages.NO_PERMISSION));
                }
                ...
                if (sender.hasPermission(this.permsReset)) {
                    ...
                    if (chairCount > 0) {
                        sender.sendMessage(Messages.getPrefix() + " §aSuccessfully removed §6" + chairCount
                                + " players§a from their chairs");
                    } else {
        ...
        if (showHelp) {
            sender.sendMessage(Messages.getPrefix() + " §3/" + cmd.getName()
                    + " toggle §7(§eor §3/bct§7, §3/toggleChairs§7)"); // TODO: allow player(+ @a,@r,@p) as arg
            sender.sendMessage(Messages.getPrefix() + " §3/" + cmd.getName()
                    + " on§7/§3off §7(§eor §3/bct on§7/§3off§7)"); // TODO: allow player(+ @a,@r,@p) as arg
            sender.sendMessage(Messages.getPrefix() + " §3/" + cmd.getName() + " reload§7/§3rl");
            sender.sendMessage(
                    Messages.getPrefix() + " §3/" + cmd.getName() + " reset"); // TODO: allow player(+ @a,@r,@p) as arg
        }
