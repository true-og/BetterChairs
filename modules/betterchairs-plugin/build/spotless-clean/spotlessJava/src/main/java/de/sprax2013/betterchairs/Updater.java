...
public class Updater implements Listener {
    ...
    private void checkForUpdates() throws IOException {
        ...
        if (isNewerVersion(currVersion, versionStr)) {
            ...
            ChairManager.getLogger()
                    .info(() -> String.format(
                            "Found a new update v%s -> v%s — Download the update from: %nmodrinth: %s%nHangar: %s%nSpigotMC: %s%nGitHub: %s%n%nChangelog: %s",
                            currVersion,
                            versionStr,
                            MODRINTH_URL,
                            HANGAR_URL,
    ...
    private void onJoin(PlayerJoinEvent e) {
        ...
        e.getPlayer()
                ...
                .sendMessage(new ComponentBuilder("[")
                        ...
                        .append(BetterChairsPlugin.getInstance().getName())
                        .color(ChatColor.GOLD)
                        .append("] ")
                        .color(ChatColor.GRAY)
                        .append("Found a new update v"
                                + this.plugin.getDescription().getVersion() + " -> v" + this.newerVersion)
                        .color(ChatColor.YELLOW)
                        .append(" [modrinth] ")
                        .color(ChatColor.GREEN)
                        .event(new ClickEvent(ClickEvent.Action.OPEN_URL, MODRINTH_URL))
                        .event(new HoverEvent(
                                HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("§2Click to visit the download page on modrinth")))
                        .append(" [Hangar] ")
                        .color(ChatColor.GREEN)
                        .event(new ClickEvent(ClickEvent.Action.OPEN_URL, HANGAR_URL))
                        .event(new HoverEvent(
                                HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("§2Click to visit the download page on Hangar")))
                        .append(" [SpigotMC] ")
                        .color(ChatColor.GREEN)
                        .event(new ClickEvent(ClickEvent.Action.OPEN_URL, SPIGOT_MC_URL))
                        .event(new HoverEvent(
                                HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("§2Click to visit the download page on SpigotMC")))
                        .append("[GitHub] ")
                        .color(ChatColor.GREEN)
                        .event(new ClickEvent(ClickEvent.Action.OPEN_URL, GITHUB_URL))
                        .event(new HoverEvent(
                                HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("§2Click to visit the download page on GitHub")))
                        .append("[Changelog]")
                        .color(ChatColor.DARK_GREEN)
                        .event(new ClickEvent(ClickEvent.Action.OPEN_URL, getChangelogUrl(this.newerVersion)))
                        .event(new HoverEvent(
                                HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("§aClick to see the Changelogs at GitHub.com")))
                        .create());
    }
