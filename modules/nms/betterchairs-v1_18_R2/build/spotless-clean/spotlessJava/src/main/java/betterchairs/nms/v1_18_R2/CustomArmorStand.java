...
class CustomArmorStand extends EntityArmorStand implements CustomChairEntity {
    ...
    public void k() { // tick
        ...
        if (ChairUtils.didChairEntityMove(
                this.expectedLocation, this.dc() /* locX */, this.de() /* locY */, this.di() /* locZ */)) {
            this.expectedLocation.setY(Math.min(this.de() /* locY */, this.expectedLocation.getY()));

            this.m(
                    this.expectedLocation.getX(),
                    this.expectedLocation.getY(),
                    this.expectedLocation.getZ()); // teleportAndSync
        }
